...
get_vision_zero_data <- function (rate_limit = 1000, offset = 0) {
  if (rate_limit > 1000) {
    warning(
  ...
  url <- "https://maps2.dcgis.dc.gov/dcgis/rest/services/DDOT/VisionZero/FeatureServer/0/query?outFields=*&where=1%3D1&f=geojson"
  ...
  results_list |> 
    ...
    mutate(
      REQUESTDATE = lubridate::as_datetime(
        (REQUESTDATE / 1000) - (6 * 60 * 60), tz = "UTC"
      )
    )
}
